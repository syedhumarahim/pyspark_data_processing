...
def test_extract():
    ...
    file_path = extract()
    assert os.path.exists(file_path) is True, "The data file should exist after extraction."
...
def test_load_data(spark):
    ...
    assert df.count() > 0, "DataFrame should have at least one row."
    assert "Patient ID" in df.columns, "Expected 'Patient ID' column to be present."
...
def test_example_transform(spark):
    ...
    assert "RiskCategory" in transformed_df.columns, "Expected 'RiskCategory' column after transformation."
